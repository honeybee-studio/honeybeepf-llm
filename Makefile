...
# Default target
help:
	@echo "honeybeepf-llm Build Commands"
	@echo ""
	...
	@echo "  lint           Run clippy on common + userspace crates (macOS-safe)"
	@echo "  lint-fix       Run clippy auto-fix on common + userspace crates"
...
# Build commands
build:
	cargo build -p honeybeepf-llm

build-release:
	cargo build --release -p honeybeepf-llm
...
lint:
	cargo clippy -p honeybeepf-llm-common -p honeybeepf-llm

# Lint with auto-fix
lint-fix:
	cargo clippy --fix -p honeybeepf-llm-common -p honeybeepf-llm --allow-dirty --allow-staged --allow-no-vcs
...
lint-fix-all:
	cargo clippy --fix --all-targets --all-features --allow-dirty --allow-staged --allow-no-vcs

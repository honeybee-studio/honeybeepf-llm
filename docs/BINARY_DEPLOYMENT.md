# honeybeepf-llm Binary Deployment Guide

honeybeepf-llm is an eBPF-based LLM observability agent. It intercepts TLS traffic to capture LLM API usage (model, tokens, latency) without code changes or proxies.
...
# Binary location
ls -la target/release/honeybeepf-llm
```
...
cargo xtask deploy --host root@<your-server> --path /opt/honeybeepf-llm/bin/honeybeepf-llm --release
```
...
# 2. Copy binary
scp target/release/honeybeepf-llm <user>@<your-server>:/tmp/

# 3. Install on server
ssh <user>@<your-server>
sudo mv /tmp/honeybeepf-llm /usr/local/bin/
sudo chmod +x /usr/local/bin/honeybeepf-llm
```
...
scp dist/honeybeepf-llm-*.tar.gz <user>@<your-server>:/tmp/
ssh <user>@<your-server> "cd /tmp && tar xzf honeybeepf-llm-*.tar.gz && cd honeybeepf-llm-* && sudo ./install.sh"
```
...
```bash
sudo /usr/local/bin/honeybeepf-llm
```
...
sudo setcap 'cap_sys_admin,cap_bpf,cap_perfmon+ep' /usr/local/bin/honeybeepf-llm

# Run as regular user
/usr/local/bin/honeybeepf-llm
```
...
| `CAP_PERFMON` | Access perf events for uprobes |
...
```bash
sudo tee /etc/systemd/system/honeybeepf-llm.service << 'EOF'
[Unit]
Description=honeybeepf-llm LLM eBPF Observability Agent
...
```bash
sudo mkdir -p /etc/honeybeepf-llm
sudo tee /etc/honeybeepf-llm/honeybeepf-llm.env << 'EOF'
# honeybeepf-llm Configuration
# RUST_LOG=info
...
# Check status
sudo systemctl status honeybeepf-llm

# View logs
sudo journalctl -u honeybeepf-llm -f
```
...
# Run in verbose mode
sudo /usr/local/bin/honeybeepf-llm --verbose
```
...
sudo systemctl stop honeybeepf-llm
sudo systemctl disable honeybeepf-llm

# Delete files
sudo rm /etc/systemd/system/honeybeepf-llm.service
sudo rm /usr/local/bin/honeybeepf-llm
sudo rm -rf /etc/honeybeepf-llm

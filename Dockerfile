...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    cargo chef cook --release --recipe-path recipe.json --package honeybeepf-llm --target $TARGET_TRIPLE
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    cargo build --release --package honeybeepf-llm --target $TARGET_TRIPLE && \

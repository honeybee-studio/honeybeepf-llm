...
**File:** `honeybeepf-llm-common/src/lib.rs`
...
**Location:** `honeybeepf-llm-ebpf/src/probes/builtin/`
...
use honeybeepf_llm_common::{EventMetadata, MyBuiltinEvent};
...
#[tracepoint]
pub fn honeybeepf_llm_my_probe(ctx: TracePointContext) -> u32 {
...
**Location:** `honeybeepf-llm/src/probes/builtin/`
...
use anyhow::Result;
use aya::Bpf;
use honeybeepf_llm_common::MyBuiltinEvent;
use log::info;
...
impl Probe for MyBuiltinProbe {
    fn attach(&self, bpf: &mut Bpf) -> Result<()> {
        ...
        attach_tracepoint(
            bpf,
            TracepointConfig {
                program_name: "honeybeepf_llm_my_probe", // Must match kernel function name
...
**File:** `honeybeepf-llm/src/lib.rs`

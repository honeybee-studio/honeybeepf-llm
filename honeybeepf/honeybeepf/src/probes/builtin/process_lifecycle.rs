...
impl Probe for ProcessLifecycleProbe {
    fn attach(&self, bpf: &mut Ebpf, _resolver: IdentityResolver) -> Result<()> {
        ...
        let exit_prog: &mut KProbe = bpf
            .program_mut("honeybeepf_process_exit")
            .context("BPF program honeybeepf_process_exit not found")?
            .try_into()?;
        exit_prog.load()?;
        ...
        let oom_prog: &mut KProbe = bpf
            .program_mut("honeybeepf_oom_kill_process")
            .context("BPF program honeybeepf_oom_kill_process not found")?
            .try_into()?;
        oom_prog.load()?;
